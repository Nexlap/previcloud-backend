...
function tabellaVoci(sfondoHeader, testoHeader, sfondoRiga, sfondoAlt, testoPrimario, testoSecondario, fontFamily) {
  ...
  function tabellaVociGriglia() {
    const fontFamily = "'DM Sans',Arial,sans-serif"
    const cellBorder = 'border:0.5px solid #E5E7EB'
    const accentDot = `<span style="display:inline-block;width:5px;height:5px;background:${coloreHex};margin-right:8px;vertical-align:middle;position:relative;top:-1px"></span>`
    if (p.voci.length === 0) return `<div style="font-family:${fontFamily};font-size:13px;white-space:pre-wrap;color:#1a1a2e;line-height:1.9">${testoEscapato}</div>`
    if (nascondiPrezzi) {
      const righe = p.voci.map((v) => {
        const dettagliHtml = v.dettagli && v.dettagli.length > 0
          ? `<div style="margin-top:5px;padding-left:13px">${v.dettagli.map(d => `<div style="font-size:11px;color:#6B7280;padding-left:4px"> ${d}</div>`).join('')}</div>`
          : ''
        const descHtml = v.descrizione ? `<div style="font-size:11px;color:#6B7280;margin-top:3px;font-style:italic;padding-left:13px">${v.descrizione}</div>` : ''
        return `<tr data-paginate="servizio-row"><td style="padding:10px 14px;${cellBorder};font-size:13px;color:#1a1a2e;vertical-align:top">${accentDot}<strong>${v.nome}</strong>${descHtml}${dettagliHtml}</td></tr>`
      }).join('')
      return `<div data-section="servizi"><table style="width:100%;border-collapse:collapse;font-family:${fontFamily};margin-bottom:20px"><thead><tr><th style="padding:10px 14px;${cellBorder};font-size:11px;font-weight:700;color:#fff;text-align:left;letter-spacing:1px;text-transform:uppercase;background:#1a1a2e">Servizio incluso</th></tr></thead><tbody>${righe}</tbody></table></div>`
    }
    const righe = p.voci.map((v) => {
      const dettagliHtml = v.dettagli && v.dettagli.length > 0
        ? `<div style="margin-top:5px;padding-left:13px">${v.dettagli.map(d => `<div style="font-size:11px;color:#6B7280;padding-left:4px"> ${d}</div>`).join('')}</div>`
        : ''
      const descHtml = v.descrizione ? `<div style="font-size:11px;color:#6B7280;margin-top:3px;font-style:italic;padding-left:13px">${v.descrizione}</div>` : ''
      return `<tr data-paginate="servizio-row"><td style="padding:10px 14px;${cellBorder};font-size:13px;color:#1a1a2e;vertical-align:top">${accentDot}<strong>${v.nome}</strong>${descHtml}${dettagliHtml}</td><td style="padding:10px 14px;${cellBorder};font-size:13px;color:#6B7280;text-align:right;vertical-align:top">${v.prezzo ? '' + v.prezzo : ''}</td><td style="padding:10px 14px;${cellBorder};font-size:13px;color:#1a1a2e;text-align:right;font-weight:600;vertical-align:top">${v.totale ? '' + v.totale : ''}</td></tr>`
    }).join('')
    return `<div data-section="servizi"><table style="width:100%;border-collapse:collapse;font-family:${fontFamily};margin-bottom:20px"><thead><tr><th style="padding:10px 14px;${cellBorder};font-size:11px;font-weight:700;color:#fff;text-align:left;letter-spacing:1px;text-transform:uppercase;background:#1a1a2e">Servizio</th><th style="padding:10px 14px;${cellBorder};font-size:11px;font-weight:700;color:#fff;text-align:right;letter-spacing:1px;text-transform:uppercase;background:#1a1a2e">Prezzo</th><th style="padding:10px 14px;${cellBorder};font-size:11px;font-weight:700;color:#fff;text-align:right;letter-spacing:1px;text-transform:uppercase;background:#1a1a2e">Totale</th></tr></thead><tbody>${righe}</tbody></table></div>`
  }

  function riepilogoTotaliGriglia() {
    if (!p.totale && !p.imponibile) return ''
    const fontSans = "'DM Sans',Arial,sans-serif"
    const fontSerif = "'Fraunces',Georgia,serif"
    return `<div style="display:flex;justify-content:flex-end;margin-top:8px"><div style="border:1px solid #E5E7EB;border-radius:8px;overflow:hidden;min-width:240px;font-family:${fontSans}">${p.imponibile ? `<div style="display:flex;justify-content:space-between;padding:10px 16px;font-size:12px;color:#6B7280;border-bottom:0.5px solid #E5E7EB"><span>Imponibile</span><span>${p.imponibile}</span></div>` : ''}${p.iva ? `<div style="display:flex;justify-content:space-between;padding:10px 16px;font-size:12px;color:#6B7280;border-bottom:0.5px solid #E5E7EB"><span>${p.iva.split(':')[0]}</span><span>${p.iva.split(':')[1] ? p.iva.split(':')[1].trim() : ''}</span></div>` : ''}${p.totaleLordo ? `<div style="display:flex;justify-content:space-between;padding:8px 16px;font-size:12px;color:#9CA3AF;text-decoration:line-through;border-bottom:0.5px solid #E5E7EB"><span>Totale</span><span>${p.totaleLordo}</span></div>` : ''}${p.sconto ? `<div style="display:flex;justify-content:space-between;padding:8px 16px;font-size:12px;color:#0E9F8E;font-weight:600;border-bottom:0.5px solid #E5E7EB"><span>Sconto</span><span>-${p.sconto}</span></div>` : ''}${p.totale ? `<div style="display:flex;justify-content:space-between;align-items:center;padding:12px 16px;background:${coloreHex};color:#fff;font-size:15px;font-weight:700"><span>TOTALE</span><span style="font-family:${fontSerif};font-size:18px;font-weight:600">${p.totale}</span></div>` : ''}</div></div>`
  }
  ...
  function stilePageFooterTemplate(tmpl, coloreHex) {
    ...
    const map = {
      ...
      griglia: `${base}font-size:11px;color:#9CA3AF;border-top:1px solid #E5E7EB;`,
    }
  ...
  const templates = {
    ...
    griglia: `<style>@import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700&family=Fraunces:opsz,wght@9..144,400;9..144,600;9..144,700&display=swap');*{box-sizing:border-box;margin:0;padding:0}body{font-family:'DM Sans',Arial,sans-serif;padding:48px;color:#1a1a2e;background:#fff;font-size:13px;line-height:1.6}</style><div data-paginate="intro-block"><div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:24px;padding-bottom:20px;border-bottom:1px solid #E5E7EB"><div>${logoHtml ? `<div style="margin-bottom:10px">${logoHtml}</div>` : ''}<div style="font-size:20px;font-weight:700;color:${coloreHex};letter-spacing:-0.3px">${nome}</div><div style="font-size:11px;color:#888;margin-top:4px;line-height:1.8">${contattiAzienda}</div></div><div style="font-family:'Fraunces',Georgia,serif;font-size:30px;font-weight:600;color:#1a1a2e;letter-spacing:0.5px;line-height:1">PREVENTIVO</div></div><div style="display:flex;gap:12px;margin-bottom:24px"><div style="flex:1;padding:12px 14px;border:1px solid ${coloreHex};border-radius:6px"><div style="font-size:10px;font-weight:700;color:#9CA3AF;letter-spacing:1px;text-transform:uppercase;margin-bottom:4px">Data</div><div style="font-size:13px;font-weight:600;color:#1a1a2e">${data}</div></div><div style="flex:1;padding:12px 14px;border:1px solid ${coloreHex};border-radius:6px"><div style="font-size:10px;font-weight:700;color:#9CA3AF;letter-spacing:1px;text-transform:uppercase;margin-bottom:4px">N. preventivo</div><div style="font-size:13px;font-weight:600;color:#1a1a2e">${numeroPreventivo || '—'}</div></div><div style="flex:1;padding:12px 14px;border:1px solid ${coloreHex};border-radius:6px"><div style="font-size:10px;font-weight:700;color:#9CA3AF;letter-spacing:1px;text-transform:uppercase;margin-bottom:4px">Validità</div><div style="font-size:13px;font-weight:600;color:#1a1a2e">${p.validita || '30 giorni'}</div></div></div>${clienteDati ? `<div style="margin-bottom:24px;border-radius:8px;overflow:hidden;border:1px solid #E5E7EB"><div style="background:#1a1a2e;padding:10px 16px"><div style="font-size:10px;font-weight:700;color:#fff;letter-spacing:1px;text-transform:uppercase">Cliente</div></div><div style="padding:14px 16px;background:#f8f9fa"><div style="font-size:14px;font-weight:600;color:#1a1a2e">${clienteDati.nome}</div>${clienteDati.indirizzo ? `<div style="font-size:12px;color:#6B7280;margin-top:2px">${clienteDati.indirizzo}</div>` : ''}${clienteDati.email ? `<div style="font-size:12px;color:#6B7280;margin-top:1px">${clienteDati.email}</div>` : ''}${clienteDati.telefono ? `<div style="font-size:12px;color:#6B7280;margin-top:1px">${clienteDati.telefono}</div>` : ''}</div></div>` : ''}${p.problema ? `<div style="background:#f8f9fa;border-left:3px solid ${coloreHex};padding:12px 16px;border-radius:0 6px 6px 0;margin-bottom:24px;font-size:13px;color:#444">${p.problema}</div>` : ''}</div>${tabellaVociGriglia()}${bloccoFooter(`${rimborsiHtml}${riepilogoTotaliGriglia()}${p.note ? `<div style="margin-top:24px;padding:12px 16px;background:#fffbeb;border:1px solid #fde68a;border-radius:6px;font-size:12px;color:#92400e"><strong>Condizioni:</strong> ${p.note}</div>` : ''}${extraPagamentoHtml}${p.pagamento ? `<div style="margin-top:12px;padding:12px 16px;background:#f0fdf4;border:1px solid #86efac;border-radius:6px;font-size:12px;color:#166534"><strong> Pagamento:</strong> ${p.pagamento}</div>` : ''}${notePagamento ? `<div style="margin-top:12px;padding:12px 16px;background:#f8f9fa;border-radius:6px;font-size:12px;color:#6B7280"> ${notePagamento}</div>` : ''}`)}`,
  }
